...
// --- DIRECT IMPORTS FOR TERMUX STABILITY ---
...
const Index = () => {
  ...
  return (
    <div className={cn(
      "min-h-screen bg-[#000] text-white selection:bg-purple-600 font-sans overflow-x-hidden transition-colors duration-1000 relative",
      gravityOn ? "cursor-crosshair" : "cursor-default"
    )}>

      {/* 🧬 Z-INDEX 1000: QUANTUM BIOMETRIC OVERLAY (TOP LAYER) */}
      <AnimatePresence>
        {isScanning && (
          <motion.div
      ...
      {/* 🌌 Z-INDEX 0: BACKGROUND LAYERS (BOTTOM LAYER) */}
      ...
      {/* 🍎 Z-INDEX 1: GRAVITY SCENE */}
      <div ref={sceneRef} className="fixed inset-0 z-[1] pointer-events-none" />

      {/* 🔮 Z-INDEX 100: TOP HUD PROGRESS */}
      <motion.div className="fixed top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-purple-600 via-white to-purple-600 z-[100] origin-left" style={{ scaleX }} />

      {/* 🛰️ Z-INDEX 100: NAV-BAR */}
      <nav className="fixed top-0 w-full z-[100] px-8 py-6 flex justify-between items-center backdrop-blur-xl border-b border-white/5 bg-black/40">
        <div className="flex items-center gap-6">
          ...
          <div className="hidden md:flex flex-col">
            <span className="text-[8px] font-black tracking-[4px] text-purple-500 uppercase opacity-70">Neural Core v10.5</span>
        ...
        <motion.div style={{ y: y1 }} className="max-w-7xl mx-auto text-center">
          ...
          <div className="flex flex-col md:flex-row gap-10 justify-center items-center mt-12">
            <motion.div animate={{ x: position.x, y: position.y }} transition={{ type: "spring", stiffness: 150, damping: 15 }}>
                ...
                className="bg-white text-black px-24 py-16 rounded-[40px] font-black text-3xl hover:bg-purple-600 hover:text-white transition-all transform shadow-[0_40px_100px_rgba(124,58,237,0.2)]"
                >
      ...
      {/* 🤖 Z-INDEX 500: NEURAL AI CONCIERGE */}
      ...
      <footer className="py-48 px-12 border-t border-white/5 mt-64 bg-zinc-950/50 backdrop-blur-md relative z-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end gap-20">
           ...
           <div className="flex flex-col items-end gap-12">
              <div className="flex gap-10">
                <Globe className="text-zinc-500 hover:text-purple-500 transition-colors cursor-pointer" size={28} />
                <Layers className="text-zinc-500 hover:text-purple-500 transition-colors cursor-pointer" size={28} />
                <Terminal className="text-zinc-500 hover:text-purple-500 transition-colors cursor-pointer" size={28} />
              </div>
